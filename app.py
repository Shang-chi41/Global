...
try:
    ...
    theme_config = db["Theme_Config"]  # 🆕 Collection cho theme
    ...
    print("   - Theme_Config: cấu hình màu sắc")
...
# ─────────────────────────────────────────────
def get_theme():
    """Lấy theme từ MongoDB, trả về dict hoặc None"""
    if theme_config is not None:
        doc = theme_config.find_one({"_id": "current"})
        if doc:
            del doc["_id"]
            return doc
    return None
...
# ─────────────────────────────────────────────
#  API: THEME (LƯU VÀO MONGODB)
# ─────────────────────────────────────────────
@app.route("/api/theme", methods=["GET", "POST"])
@login_required
def api_theme():
    """Lấy hoặc lưu theme từ MongoDB"""
    ...
    if request.method == "GET":
        ...
        data["updated_at"] = (datetime.utcnow() + timedelta(hours=7)).isoformat()
        theme_config.update_one({"_id": "current"}, {"$set": data}, upsert=True)
        return jsonify({"status": "ok", "message": "Theme đã được lưu"})
...
# ─────────────────────────────────────────────
#  PAGE ROUTES (có theme)
# ─────────────────────────────────────────────
@app.route("/")
@login_required
def home():
    return render_template("base.html", username=session.get("username", "admin"), theme=get_theme())

@app.route("/control")
@login_required
def control():
    return render_template("control.html", username=session.get("username", "admin"), theme=get_theme())

@app.route("/monitor")
@login_required
def monitor():
    return render_template("monitor.html", username=session.get("username", "admin"), theme=get_theme())

@app.route("/settings")
@login_required
def settings():
    return render_template("settings.html", username=session.get("username", "admin"), theme=get_theme())

# ─────────────────────────────────────────────
#  STATIC FILES (CHO UNITY WEBGL)
# ─────────────────────────────────────────────
